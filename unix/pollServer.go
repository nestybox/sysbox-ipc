...
// Processes out-of-band 'wakeup' events generated by pollserver clients. Callers
// of this method must acquire pollServer's mutex.
func (ps *PollServer) processWakeupEvent() {

	var buf [100]byte
	ps.wakeupReader.Read(buf[0:])
	//logrus.Errorf("wakeup routine read %s", string(buf))
...
// Processes 'pollin' events received through one of the monitored fds. Callers
// of this method must acquire pollServer's mutex.
...
// Processes 'pollout' events received through one of the monitored fds. Callers
// of this method must acquire pollServer's mutex.
